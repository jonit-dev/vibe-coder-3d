...
pub fn create_bvh_test_scene() {
    ...
    println!("📦 Registering {} meshes with BVH system...", test_meshes.len());
    ...
    for (entity_id, positions, indices) in test_meshes {
        ...
        println!("   📋 Mesh {}: {} triangles, offset: ({:.1}, {:.1}, {:.1})",
            entity_id, indices.len(), offset.x, offset.y, offset.z);
    }
    ...
    println!("   ⏱️ Build time: {:.3}ms", stats.metrics.mesh_build_time_ms);
    ...
    for (i, (origin, direction)) in test_rays.iter().enumerate() {
        if let Some(hit) = bvh_manager.raycast_first(*origin, *direction, 100.0) {
            hit_count += 1;
            println!("   🎯 Ray {}: HIT entity {} at distance {:.3}", i + 1, hit.entity_id, hit.distance);
        } else {
    ...
    println!("   👁️ Visible meshes: {}", metrics.visible_meshes_last_frame);
    ...
    println!("   📈 Raycasting efficiency: {:.1}% reduction in triangle tests", efficiency);
    println!("   ⚡ BVH acceleration: {:.1}x faster than brute force", total_triangles / ray_triangle_tests.max(1.0));

    println!("\n🎉 === BVH Integration Demo Complete ===");
    println!("✅ BVH system successfully integrated with rendering pipeline!");
    println!("✅ Real-time frustum culling working: {}/{} meshes visible", visible_entities.len(), 4);
    println!("✅ Accelerated raycasting working: {}/4 rays hit", hit_count);
    println!("✅ Performance gains achieved: {:.1}% efficiency improvement", efficiency);
}
