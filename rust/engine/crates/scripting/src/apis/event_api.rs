...
pub fn register_event_api(lua: &Lua, entity_id: u32) -> LuaResult<()> {
    ...
    let on_fn = lua.create_function(move |lua, (_self, event_type, handler): (Table, String, Function)| {
        ...
        let handler_wrapper: EventHandler = Arc::new(move |_args: Vec<Value>| {
            ...
            log::debug!("[Event API] Handler {} registered for '{}'", handler_id, event_type_for_log);
        });
    ...
    let off_fn = lua.create_function(move |lua, (_self, event_type, handler_id): (Table, String, u64)| {
    ...
    let emit_fn = lua.create_function(move |lua, (_self, event_type, payload): (Table, String, Value)| {
        // Get all registered handlers for this event type
        let handler_count = {
            let bus = EVENT_BUS.lock().unwrap();
            bus.get(&event_type).map(|handlers| handlers.len()).unwrap_or(0)
        };
...
pub fn cleanup_event_api(lua: &Lua, entity_id: u32) -> LuaResult<()> {
    log::debug!("[Event API] Cleaning up event handlers for entity {}", entity_id);
