...
fn test_lod_path_resolution() -> anyhow::Result<()> {
    ...
    let original_path = lod_manager.get_lod_path("models/test.glb", Some(vibe_coder_engine::renderer::lod_manager::LODQuality::Original));
    assert_eq!(original_path, "models/test.glb");

    // Test high quality path
    let high_path = lod_manager.get_lod_path("models/test.glb", Some(vibe_coder_engine::renderer::lod_manager::LODQuality::HighFidelity));
    assert_eq!(high_path, "models/lod/test_high.glb");

    // Test low quality path
    let low_path = lod_manager.get_lod_path("models/test.glb", Some(vibe_coder_engine::renderer::lod_manager::LODQuality::LowFidelity));
    assert_eq!(low_path, "models/lod/test_low.glb");

    // Test path with different extensions
    let gltf_path = lod_manager.get_lod_path("assets/cube.gltf", Some(vibe_coder_engine::renderer::lod_manager::LODQuality::HighFidelity));
...
fn test_lod_manager_configuration() -> anyhow::Result<()> {
    ...
    assert_eq!(lod_manager.get_quality(), vibe_coder_engine::renderer::lod_manager::LODQuality::HighFidelity);
    ...
    let handle = std::thread::spawn(move || {
        manager_clone.set_quality(vibe_coder_engine::renderer::lod_manager::LODQuality::LowFidelity);
    });

    handle.join();

    assert_eq!(lod_manager.get_quality(), vibe_coder_engine::renderer::lod_manager::LODQuality::LowFidelity);
...
fn test_lod_component() -> anyhow::Result<()> {
    ...
    assert_eq!(lod_component.high_fidelity_path, Some("models/lod/test_high.glb".to_string()));
    assert_eq!(lod_component.low_fidelity_path, Some("models/lod/test_low.glb".to_string()));
