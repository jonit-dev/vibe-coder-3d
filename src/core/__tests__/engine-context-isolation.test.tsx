...
const TestComponent = ({ testId, onStateChange }: { testId: string; onStateChange?: (state: any) => void }) => {
  const controls = useGameEngineControls();
  const loopStore = useLoopStore();
  const state = loopStore((s) => ({ isRunning: s.isRunning, isPaused: s.isPaused }));
  ...
  return (
    <div data-testid={testId}>
      <button onClick={controls.startEngine} data-testid={`${testId}-start`}>Start</button>
      <button onClick={controls.pauseEngine} data-testid={`${testId}-pause`}>Pause</button>
      <button onClick={controls.stopEngine} data-testid={`${testId}-stop`}>Stop</button>
...
describe('EngineProvider Isolation', () => {
  it('should create isolated loop stores for each provider', () => {
    let engineA: any = null;
    let engineB: any = null;

    render(
      <div>
        <EngineProvider>
          <TestComponent testId="engineA" onStateChange={(state) => { engineA = state; }} />
        </EngineProvider>
        <EngineProvider>
          <TestComponent testId="engineB" onStateChange={(state) => { engineB = state; }} />
        </EngineProvider>
      </div>
    );

    // Both engines should start in stopped state
    expect(engineA).toEqual({ isRunning: false, isPaused: false });
    expect(engineB).toEqual({ isRunning: false, isPaused: false });
  });
