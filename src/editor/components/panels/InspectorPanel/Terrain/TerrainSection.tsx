    ...
    >
      <CollapsibleSection title="Dimensions" defaultOpen>
        <div className="grid grid-cols-2 gap-2">
          <SingleAxisField
            label="Width"
            value={terrain.size[0]}
            min={1}
            step={1}
            onChange={(v) => onUpdate({ size: [v, terrain.size[1]] })}
          />
          <SingleAxisField
            label="Depth"
            value={terrain.size[1]}
            min={1}
            step={1}
            onChange={(v) => onUpdate({ size: [terrain.size[0], v] })}
          />
          <SingleAxisField
            label="Segments X"
            value={terrain.segments[0]}
            min={2}
            step={1}
            onChange={(v) =>
              onUpdate({ segments: [Math.max(2, Math.floor(v)), terrain.segments[1]] })
            }
          />
          <SingleAxisField
            label="Segments Z"
            value={terrain.segments[1]}
            min={2}
            step={1}
            onChange={(v) =>
              onUpdate({ segments: [terrain.segments[0], Math.max(2, Math.floor(v))] })
            }
          />
          <SingleAxisField
            label="Height Scale"
      ...
      <CollapsibleSection title="Noise" defaultOpen>
